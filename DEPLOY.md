...
**`ALLOWED_USER_KEYS` (אינטק, Phase 5) הוא היוצא מן הכלל:** הוא נקרא עם `os.environ.get` ולא מפיל את הקונטיינר כשהוא חסר. הכישלון שלו שקט אחרת — משתמש חדש מקבל "אני לא מכיר אותך" והאינטק לעולם לא מתחיל, בלי שורת שגיאה אחת. מוסיפים לשם את ה-user key המלא (`telegram_<id>`), מופרד בפסיקים.
